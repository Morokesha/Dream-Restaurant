...
namespace PW
{
    ...
    public class ProductManagerEditor : Editor
    {
        ...
        private SerializedProperty placeHolderPrefab;

        private SerializedProperty Products;

        private SerializedProperty ProductScreenshotsPath;
        ...
        private void DrawPropertyArray(SerializedProperty property, ref bool fold)
        {
            ...
            if (fold)
            {
                ...
                {
                    var curIndex = property.arraySize;
            ...
            EditorGUILayout.HelpBox(
                "Gameplay prefabs are created depending on ProductTypes, these created prefabs will contain scripts and helpers depending on their gameplay.",
                MessageType.Info);
            //Button to create gameplay prefabs.


            if (GUILayout.Button("Generate Gameplay Prefabs")) GenerateGameplayPrefabs(property, arraySizeProp);
            ...
            EditorGUILayout.HelpBox(
                "This path should be the same path where you saved your screenshots with the ScreenShotHelper component in the ScreenshotScene. Whenever you add, or change a new product, you should assign screenshots below.",
                MessageType.Info);
            ProductScreenshotsPath.stringValue =
                EditorGUILayout.TextField("ScreenshotsPath", ProductScreenshotsPath.stringValue);
            EditorGUI.indentLevel++;
            ...
            if (GUILayout.Button(new GUIContent("AssignScreenShots",
                    "This button assigns the screenshots to the products using the prefab name of products, if you changed the prefab in your screenshot, you neeed to generate screenshots again,before doing that.")))
                AssignScreenShotsToOrders(serializedObject, property, arraySizeProp.intValue,
                    ProductScreenshotsPath.stringValue);
            EditorGUI.indentLevel--;
        }

        private static void AssignScreenShotsToOrders(SerializedObject serializedObject, SerializedProperty property,
            int totalSize, string ScreenShotPath)
        {
            var orderGen = FindObjectOfType<OrderGenerator>();
            if (orderGen == null)
            {
                Debug.LogError(
                    "There is no OrderGenerator in the scnee. To assign screenshots to orders, you need to add OrderGenerator component to a gameObject in the scene.");
                return;
            }
            ...
            for (var i = 0; i < totalSize; i++)
            {
                ...
                if (curProduct.FindPropertyRelative("dontIncludeInThisScene").boolValue) continue;
                ...
                var serveAsDifferent =
                    curProduct.FindPropertyRelative("servedAsDifferentGameObject").objectReferenceValue;
                ...
                var orderID = curProduct.FindPropertyRelative("orderID").intValue;
        ...
        private static void GenerateGameplayPrefabs(SerializedProperty property, SerializedProperty arraySizeProp)
        {
            var PrefabPath = "Assets/CoffeeShopStarterPack/Prefabs/Gameplay/Products/";
            ...
            for (var i = 0; i < arraySizeProp.intValue; i++)
            {
                ...
                var enumIndex = serializedProductType.enumValueIndex;
                var pType = (ProductType) enumIndex;

                var pOrderID = product.FindPropertyRelative("orderID").intValue;

                //Get the prefab from the product
                var prefab = product.FindPropertyRelative("productPrefab").objectReferenceValue as GameObject;
                ...
                switch (pType)
                {
                    ...
                    case ProductType.Drinkable:
                        ...
                        for (var j = 0; j < newPrefabGo.transform.childCount; j++)
                        {
                ...
                var productMonoBehaviour = newPrefabGo.GetComponent<ProductGameObject>();
                ...
                productMonoBehaviour.serveAsDifferentGameObject = serveAsDifferentProp != null
                    ? (GameObject) serveAsDifferentProp.objectReferenceValue
                    : null;
                ...
                {
#if UNITY_2018_4_OR_NEWER
                    PrefabUtility.SaveAsPrefabAssetAndConnect(newPrefabGo, longPrefabName,
        ...
        private static void FindBoundsOfPrefabAndAddABoxCollider(GameObject prefab)
        {
            var prefab_bounds = prefab.transform.EncapsulateBounds();
