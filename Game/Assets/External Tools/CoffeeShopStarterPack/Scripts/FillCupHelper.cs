...
namespace PW
{
    public class FillCupHelper : MonoBehaviour
    {
        ...
        private BewerageMaker m_Machine;
        ...
        private IEnumerator FillAnimation(float amount)
        {
            fluid.gameObject.SetActive(true);
            var timeAmount = amount;
            var totalDist = 0 - fluid.transform.localPosition.y;
            var totalScale = fluid.transform.localScale.y;
            if (amount <= 0.0001f)
            {
