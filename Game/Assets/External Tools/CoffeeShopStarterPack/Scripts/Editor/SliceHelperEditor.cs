...
namespace PW
{
    ...
    {
        private SerializedProperty sliceCount;
        private SerializedProperty slicePrefab;
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (GUILayout.Button("Arrange Slices"))
            {
                ...
                if (slicePrefab != null && slicePrefab.objectReferenceValue != null)
                {
                    ...
                    var existedChildrenObjects = new List<GameObject>();
                    for (var c = 0; c < selected.transform.childCount; c++)
                    ...
                    for (var i = 0; i < sliceCount.intValue; i++)
                    {
                        var rotNow = i * 36;
                        ...
                        go.transform.rotation = Quaternion.identity;
                        var slice = Instantiate(slicePrefab.objectReferenceValue, go.transform) as GameObject;
                    ...
                    foreach (var go in existedChildrenObjects) DestroyImmediate(go);
                }
            }
